...
import { createClass, getClassDetail, updateClass } from '../../../../api/class';


const CreateUpdateClass = (props: {
  ...
  const getDetail = async () => {
    try {
      const res = await getClassDetail(props.idClass);
      const data = res.data;
      ...
      form.setFieldsValue({
        ...
        status: data.status === STATUS_CLASS.ACTIVE ? true : false, // BE dùng 1/2 → FE dùng true/false
      });
    } catch (err) {
      console.log(err);
    }
  ...
  const onSubmit = async () => {
    try {
      ...
      if (props.idClass) {
        // UPDATE
        await updateClass(props.idClass, payload);
  ...
  return (
    <ProDrawerForm
      titleHeader={props.idClass ? "Chỉnh Sửa Lớp Học" : "Tạo Lớp Học Mới"}
      subTitleHeader={
        <>
          <span>
