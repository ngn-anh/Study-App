...
class PDFService {
  ...
  async downloadExamPDF(
    ...
    try {
      ...
      if (showAlert) {
        Alert.alert('Thành công', `PDF đã được tải xuống:\n${fileName}`, [
  ...
  async previewExamPDF(
    ...
    try {
      if (downloadFirst || Platform.OS === 'ios') {
        ...
        const { path, fileName } = await this.downloadExamPDF(
          examId,
          examName,
          {
            showAlert: false,
  ...
  async showPDFOptions(
    ...
    return new Promise(resolve => {
      Alert.alert('Tải đề thi PDF', `Chọn hành động cho "${examName}":`, [
        {
          text: 'Tải về máy',
          onPress: async () => {
            try {
  ...
  async shareAppDownload(
    ...
    try {
      const defaultOptions = {
        ...
        playStoreLink:
          'https://play.google.com/store/apps/details?id=com.luyenthipro',
