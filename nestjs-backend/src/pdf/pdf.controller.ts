...
export class PdfController {
  ...
  @Get('exam/:examId/pdf')
  async downloadExamPdf(@Param('examId') examId: string, @Res() res: Response) {
    try {
      ...
      // 3. Tạo tên file
      const examName = examData.exam.name
  ...
  async previewExamPdf(@Param('examId') examId: string, @Res() res: Response) {
    try {
      const examData = await this.pdfService.findByExamId(examId);
      if (!examData) {
        throw new NotFoundException('Exam not found');
